package com.poortorich.s3.constans;

import java.util.Arrays;
import java.util.List;

public class S3Constants {

    private S3Constants() {
    }

    public static final class Config {
        public static final String BUCKET_NAME = "${cloud.aws.s3.bucket}";
        public static final String REGION = "${cloud.aws.region.static}";
        public static final String ACCESS_KEY = "${cloud.aws.credentials.accessKey}";
        public static final String SECRET_KEY = "${cloud.aws.credentials.secretKey}";
    }

    public static final class ValidationMessage {
        public static final String INVALID_IMAGE_TYPES = "지원되지 않는 이미지 형식입니다. JPG, JPEG, PNG 파일만 허용됩니다.";
        public static final String FILE_SIZE_INVALID = "파일 크기가 너무 큽니다. 5MB 이하의 파일만 허용됩니다.";
    }

    public static final class ValidationConstraint {
        public static final List<String> ALLOWED_IMAGES_TYPES = Arrays.asList(
                "image/jpeg", "image/jpg", "image/png"
        );
        public static final long MAX_FILE_SIZE = 5 * 1024 * 1024;
    }
}
